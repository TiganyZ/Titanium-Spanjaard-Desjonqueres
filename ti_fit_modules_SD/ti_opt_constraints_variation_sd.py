...
def apply_constraints(params, name_of_params):
    ...
    if name_of_params == 'bondint':
        ...
        ## Just using spanjard dejonqueres so no need.  
        ## params == pair_params
...
def pp_scale_const(LMarg, par_arr_p, ppwg, ppert, xargs, npass, maxit):
    ...
    if npass == 1:
        ...
        varwg = np.array([2., 0.1, 0.1, 0.1, 0.1]) * ( maxit - npass % maxit  ) /  maxit * np.abs(  ( ppert[-1] / np.mean(ppert)  ) )
        print('Variance for pair potential:\n %s' %(varwg))



    par_arr_p, alat_diff, clat_diff, coa_diff, min_coa, min_alat, min_vol  = pp_gaussian_variation(LMarg, xargs, par_arr_p, 
                                                                                            ppwg, varwg, npass )
    ...
    return np.abs(par_arr_p), alat_diff, clat_diff, coa_diff, min_coa, min_alat, min_vol
...
def pp_gaussian_variation(LMarg, args, par_arr_p, m_v, sig_v, npass):#, pair_pot_bank, pair_pot_bank_err):
    ...
    #print('Pair potential is:\n   %s '%(pp_args) )

    min_alat, alat_diff, min_coa, coa_diff, min_vol = lm.opt_latpars(LMarg, args, par_arr_p, a_u, a_l, alat_ideal, coa_u, coa_l, coa_ideal)
    ...
    if npass > 1:
        ...
        while cond:
            ...
            if j > 1:
                #pp_cov = g.sample_mean_covarance(pair_pot_bank, (1./pp_err1_tot**2) , pp_wgtd_mean )
                par_arr_p = ( np.random.multivariate_normal(mean=pp_wgtd_mean, cov=(pp_cov * j  + S0 * (itmax - j ) )/itmax) + par_arr_p ) / 2.
            else:
                par_arr_p =  np.random.multivariate_normal(mean=m_v, cov= S0) 
            ...
            min_alat1, alat_diff1, min_coa1, coa_diff1, min_vol1 = lm.opt_latpars(LMarg, args, par_arr_p, a_u, a_l, alat_ideal, coa_u, coa_l, coa_ideal)
            ...
            if j % 4 == 0:
                ##  Every fourth iteration, discard the worst result. 
    ...
    return par_arr_p, alat_diff1, clat_diff1, coa_diff1, min_coa1, min_alat1, min_vol1
